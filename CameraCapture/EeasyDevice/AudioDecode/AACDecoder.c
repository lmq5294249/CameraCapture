	...
	while (pkt_pos < nLen) {
		...
		if (got_frame) {
            ...
            /* 开始重采样
             ...
             使用avcodec_decode_audio4函数解码音频得到的数据类型为float 4bit，而播放器播放的格式一般为S16（signed 16bit）,这就需要对解码得到的数据进行转换。
             */
			...
			if (len > 0) {
                // 将转换后的数据复制给pFrame
                //av_get_bytes_per_sample(AV_SAMPLE_FMT_S16)返回每个样本的字节数。
